...
function render(){ 
    ...
    foods.forEach((item,index)=>{ 
        total+=item.calories; const div=document.createElement('div'); 
        div.className='item'; 
        div.innerHTML=`<span>${item.food}</span> <span>${item.calories} kcal</span> 
        <button onclick="deleteItem(${index})">
        <img src="pic/close.svg"></button>`; 
        list.appendChild(div); 
    }); 
    totalDiv.innerText='Total: '+total+' kcal'; saveTodayCalories(total);
    if(goal===null){ 
    ...
    if(goal!==null && goal>0){ 
        ...
        else goalMsg.innerText='You are within your goal'; 
    } else { 
